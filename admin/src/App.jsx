...
const DUMMY_EMPLOYEES = [
  { id: 'emp_001', firstName: 'Archie', lastName: 'Tubig', email: 'archie.tubig@gigahertz.com', dept: 'PRODUCT DEPARTMENT', loginCount: 34, createdAt: '2024-06-10T08:00:00.000Z', lastLoginAt: '2026-03-17T07:00:00.000Z', deptChangedAt: '', deptChangedBy: '', hasPhoto: false },
  { id: 'emp_002', firstName: 'Edmark', lastName: 'Lastimacruz', email: 'edmark.lastimacruz@gigahertz.com', dept: 'PRODUCT DEPARTMENT', loginCount: 55, createdAt: '2024-06-10T08:00:00.000Z', lastLoginAt: '2026-03-17T07:00:00.000Z', deptChangedAt: '', deptChangedBy: '', hasPhoto: false },
]

const DEPT_PALETTE = ['#1a3af5', '#15803d', '#d97706', '#dc2626', '#9333ea', '#0891b2', '#db2777', '#65a30d', '#0f766e', '#7c3aed', '#b45309', '#0e7490']
  ...
  return (
    <div className="layout">
      ...
      <main className="main">
        ...
        {page === 'departments' && <DepartmentsPage onViewDetails={(dept) => { setSelectedDept(dept); setPage('department-details') }} />}
        {page === 'department-details' && selectedDept && <DepartmentDetailPage dept={selectedDept} onBack={() => setPage('departments')} />}
      </main>
    </div>
  )
}
...
const TasksPage = () => {
  ...
  const handleOpenEdit = (task) => {
    setEditingTask(task)
    setFormData({ userId: task.userId || '', taskName: task.taskName || '', description: task.description || '', frequency: task.frequency || 'Weekly', priority: task.priority || 'Medium', dept: task.dept || '' })
    setShowAdd(true)
  }
  ...
  const handleSaveTask = async () => {
    ...
    if (isEditing) {
      const res = await aapi.updateEmpTask(editingTask.id, formData)
      if (res.success) {
        setTasks(tasks.map(t => (t.id === editingTask.id ? res.task : t)))
        setNotif({ msg: '✅ Task updated successfully', type: 'success' })
        resetForm()
      } else {
        setNotif({ msg: `❌ ${res.error || 'Update failed'}`, type: 'error' })
      }
      return
    }
    ...
    if (res.success) {
      ...
      setNotif({ msg: `❌ ${res.error || 'Create failed'}`, type: 'error' })
    }
  }
  ...
  const handleDeleteTask = async id => {
    ...
    if (res.success) {
      ...
      setNotif({ msg: `❌ ${res.error || 'Delete failed'}`, type: 'error' })
    }
  }
  ...
  return (
    <div className="page">
      ...
      <div className="ph">
        ...
        <button onClick={handleOpenCreate} style={{ padding: '8px 14px', background: showAdd ? 'var(--ok)' : 'var(--p)', color: 'white', border: 'none', borderRadius: 6, fontWeight: 600, fontSize: 12, cursor: 'pointer' }}>
          {showAdd ? isEditing ? '✏️ Editing' : '✏️ Add Task' : '➕ Add Task'}
        </button>
      </div>
      ...
      {showAdd && (
        <div style={{ background: 'var(--card)', border: '1px solid var(--bd)', borderRadius: 8, padding: 16, marginBottom: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            <h3 style={{ margin: 0, fontSize: 14, fontWeight: 700 }}>{isEditing ? '✏️ Edit Task' : '➕ New Task'}</h3>
            <button onClick={resetForm} style={{ fontSize: 12, color: 'var(--tx2)', background: 'none', border: 'none', cursor: 'pointer' }}>Cancel</button>
          </div>
          ...
          <button onClick={handleSaveTask} style={{ marginTop: 12, padding: '8px 14px', background: 'var(--ok)', color: 'white', border: 'none', borderRadius: 6, fontWeight: 600, fontSize: 12, cursor: 'pointer' }}>
          ...
          {filtered.map(task => (
            <div key={task.id} style={{ background: 'var(--card)', border: '1px solid var(--bd)', borderRadius: 8, padding: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => handleOpenEdit(task)} style={{ padding: '6px 10px', background: 'var(--info)', color: 'white', border: 'none', borderRadius: 4, fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>
                  ✏️ Edit
                </button>
                <button onClick={() => handleDeleteTask(task.id)} style={{ padding: '6px 10px', background: 'var(--errb)', color: 'var(--err)', border: '1px solid var(--err)', borderRadius: 4, fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>
                  🗑 Delete
                </button>
              </div>
            </div>
...
// ══ DEPARTMENTS PAGE ══
const DepartmentsPage = ({ onViewDetails }) => {
  const [users, setUsers] = useState([])
  const [tasks, setTasks] = useState([])
  const [loading, setLoading] = useState(true)
          ...
          {DEPARTMENTS.map((dept, idx) => {
            ...
            return (
              <div key={dept} style={{ background: 'var(--card)', border: '1px solid var(--bd)', borderRadius: 8, padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <button onClick={() => onViewDetails(dept)} style={{ padding: '8px 12px', background: 'var(--p)', color: 'white', border: 'none', borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>
                  View Details
                </button>
              </div>
            )
...
const DepartmentDetailPage = ({ dept, onBack }) => {
  const [users, setUsers] = useState([])
  const [tasks, setTasks] = useState([])
  const [loading, setLoading] = useState(true)
  ...
  const templates = DEPT_TASK_TEMPLATES[dept] || []

  return (
    <div className="page">
      <div className="ph" style={{ justifyContent: 'space-between' }}>
        <div>
          <h1 className="pt">🏢 {dept}</h1>
          <p className="ps">Details and tasks for this department</p>
        </div>
        <button onClick={onBack} style={{ padding: '8px 12px', borderRadius: 6, border: '1px solid var(--bd)', background: 'var(--bg)', color: 'var(--tx)', cursor: 'pointer', fontSize: 12 }}>
          ← Back to Departments
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(210px, 1fr))', gap: 14 }}>
          <div style={{ background: 'var(--card)', border: '1px solid var(--bd)', borderRadius: 8, padding: 14 }}>
            <h2 style={{ margin: 0, marginBottom: 10, fontSize: 14 }}>👥 Members</h2>
            <p style={{ margin: '0 0 10px', color: 'var(--tx2)', fontSize: 12 }}>{deptUsers.length} users in this department</p>
            <div style={{ maxHeight: 180, overflowY: 'auto' }}>
              {deptUsers.length === 0 ? <p style={{ color: 'var(--tx2)', fontSize: 12 }}>No users assigned</p> : deptUsers.map(u => (
                <div key={u.id} style={{ fontSize: 12, color: 'var(--tx)', marginBottom: 5 }}>
          ...
          <div style={{ background: 'var(--card)', border: '1px solid var(--bd)', borderRadius: 8, padding: 14 }}>
            <h2 style={{ margin: 0, marginBottom: 10, fontSize: 14 }}>✅ Department Tasks</h2>
            <p style={{ margin: '0 0 10px', color: 'var(--tx2)', fontSize: 12 }}>{deptTasks.length} assigned tasks</p>
            <div style={{ maxHeight: 180, overflowY: 'auto' }}>
              {deptTasks.length === 0 ? <p style={{ color: 'var(--tx2)', fontSize: 12 }}>No tasks assigned yet</p> : deptTasks.map(t => (
                <div key={t.id} style={{ fontSize: 12, color: 'var(--tx)', marginBottom: 4 }}>
                  {t.taskName} - {t.priority} ({t.frequency})
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ background: 'var(--card)', border: '1px solid var(--bd)', borderRadius: 8, padding: 14 }}>
            <h2 style={{ margin: 0, marginBottom: 10, fontSize: 14 }}>📋 Task Templates</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {templates.length === 0 ? <p style={{ color: 'var(--tx2)', fontSize: 12 }}>No templates available</p> : templates.map(t => (
                <div key={t} style={{ fontSize: 12, color: 'var(--tx)', paddingLeft: 6, borderLeft: '2px solid var(--p)' }}>• {t}</div>
              ))}
            </div>
          </div>
        </div>
